...
namespace Anviz\MqttClient\Client;

use DateInterval;
use DateTime;
use Anviz\MqttClient\Client\Concerns\OffersHooks;
use Anviz\MqttClient\Client\Contracts\MQTTClient as ClientContract;
use Anviz\MqttClient\Client\Contracts\Repository;
use Anviz\MqttClient\Client\Exceptions\ConnectingToBrokerFailedException;
use Anviz\MqttClient\Client\Exceptions\DataTransferException;
use Anviz\MqttClient\Client\Exceptions\PendingPublishConfirmationAlreadyExistsException;
use Anviz\MqttClient\Client\Exceptions\UnexpectedAcknowledgementException;
use Anviz\MqttClient\Client\Repositories\MemoryRepository;
use Psr\Log\LoggerInterface;

/**
 * An MQTT client implementing protocol version 3.1.
 *
 * @package Anviz\MqttClient\Client
 */
class MQTTClient implements ClientContract
{
    ...
    )
    {
        ...
        if (is_array($caFile)) {
            ...
            $this->local_cert = $caFile['local_cert'] ?? null;
            $this->local_pk = $caFile['local_pk'] ?? null;
        } else {
            $this->caFile = $caFile;
        }
    ...
    protected function establishSocketConnection(): void
    {
        if ($this->hasCertificateAuthorityFile()) {
            ...
            if(!empty($this->local_cert)){
                $ssl['local_cert'] = $this->local_cert;
            }
            if(!empty($this->local_pk)){
                $ssl['local_pk'] = $this->local_pk;
            }

            $socketContext = stream_context_create([
                'ssl' => $ssl,
            ]);
            $connectionString = 'ssl://' . $this->getHost() . ':' . $this->getPort();
    ...
    protected function performConnectionHandshake(string $username = null, string $password = null, bool $sendCleanSessionFlag = false): void
    {
        try {
            ...
            // protocol header
            $buffer .= chr(0x00);
            $i++; // length of protocol name 1
            $buffer .= chr(0x06);
            $i++; // length of protocol name 2
            $buffer .= chr(0x4d);
            $i++; // protocol name: M
            $buffer .= chr(0x51);
            $i++; // protocol name: Q
